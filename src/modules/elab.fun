functor Elab (structure Print : PRINT) : ELAB = struct
  ...
  *)
  ...
  fun restrictMorph(M.MorComp(mor1,mor2), newdom) =
    ...
    | restrictMorph(M.MorStr s, newdom) = (
        case M.structLookup s
          of M.StrDef(_,_,_,mor,_) => restrictMorph (mor, newdom)
           | M.StrDec(_,_,dom, insts,_,_) =>
              if dom = newdom then M.MorStr s
              else case M.sigRel(newdom, dom)
                     of SOME (M.Included(i, NONE)) => restrictMorph(M.MorStr(valOf (M.structMapLookup(s,i))), newdom) (* reduce to the structure generated by flattenDec *)
                      | _ => restrictFirst(inclMorphsStr insts, newdom) (* find a suitable included morphism, identity if none *)
      )
  ...
  (* filters a list of instantiations,
     ...
     post: returns the instantiations of symbols that are visible to newdom, this may be a single morphism from newdom *)
  datatype RIResult = RIDef of M.Morph | RIDec of M.SymInst list
  fun restrictInsts(insts, newdom) = restrictInsts'(insts, newdom, nil)
  and restrictInsts'(nil, newdom, found) = RIDec found
    | restrictInsts'(inst :: insts, newdom, found) =
      ...
      in case inst
         of M.InclInst(i, O, from, mor) =>
            ...
            if M.sigIncluded(newdom, from) then RIDef(restrictMorph(mor, newdom))
            else if M.sigIncluded(from, newdom) then restrictInsts'(insts, newdom, inst :: found)
            else restrictInsts'(insts, newdom, found)
          | _ => if M.sigIncluded(IDs.midOf(M.symInstCid inst), newdom)
                 then restrictInsts'(insts, newdom, inst :: found)
                 else restrictInsts'(insts, newdom, found)
      end
...
*)
  ...
  *)
  ...
  (* checks whether the ancestors of the domain of a link are covered by the link
     ...
     Therefore, the parent of dom must be included into cod.
        FR: This used to say "must be ancestor_or_selfs of cod; relaxing seems to be preserve all invariants
     If the parent of dom is an ancestor of cod, we also permit the case where cod is declared before dom if there are at most module declarations in between them.
   *)
  fun checkAncestors(dom, cod) =
    let
       ...
       (* used to be: if domPar = cod orelse List.exists (fn (a,c) => domPar = a andalso onlyModsBetween(a, IDs.lidOf c, IDs.lidOf domCid)) codAncs *)
       val _ = if List.exists (fn M.ObjSig(a,M.Ancestor p) => domPar = a andalso onlyModsBetween(a, IDs.lidOf (M.midToCid p), IDs.lidOf domCid)
                                 | M.ObjSig(d, _) => domPar = d
                                ) (M.modInclLookup cod)
               then ()
               else raise Error("the parent of the domain of a morphism must be included into the codomain")
    in
       ()
    end
  ...
  *)
              ...
              of M.SymCon(con') => (
               ...
               | M.SymStr(str') =>
                ...
                if isMeta then () else case inclMapOpt
                   ...
                   let
                      ...
                      val s = case Str
                        of M.StrDec(_,_,_,insts,_,_) => (
                           case restrictInsts(insts, dom')
                           of RIDec restrInsts => 
                                installStrDec (i', M.StrDec(Name @ [name'], q, dom', restrInsts, M.OpenDec nil, false))
                            | RIDef mor => 
                                installStrDec (i', M.StrDef(Name @ [name'], q, dom', mor, false))
                          )
                 ...
                 | SOME im =>
                    (* recursive flattening of the generated structures; just add an entry to M.structMap to realize the sharing *)
  ...
  fun flattenInst(instID : IDs.cid, installInst : M.SymInst -> IDs.cid) = flattenInst'(instID, installInst, false)
  and flattenInst'(instID, installInst, stopOnInclude) =
     let
        ...
        fun flatten1(c' : IDs.cid) = 
            case M.symLookup c'
                 of M.SymCon _ =>
                  ...
                  | M.SymIncl (M.SigIncl(dom', isMeta, _,_)) => 
                     if isMeta then () else
                     if stopOnInclude then () else
                     let
                      	val c = valOf (M.structMapLookup(s,c'))
                      	val cInst = installInst(M.StrInst(c, SOME instID, restrictMorph(mor, dom')))
                     in
                      	flattenInst'(cInst, installInst, true)
                     end
                  | M.SymMod _ => ()
     in
        M.sgnApp(dom, flatten1)
  ...
  fun flattenCase(caseid : IDs.cid, installCase : M.SymCase -> IDs.cid) = flattenCase'(caseid, installCase, false)
  and flattenCase'(caseid, installCase, stopOnInclude) =
     let
            ...
            case M.symLookup c'
                 of M.SymCon condec =>
                  ...
                  | M.SymIncl (M.SigIncl(dom', isMeta, _,_)) => 
                     ...
                     let
                      	val c = valOf (M.structMapLookup(s,c'))
                      	val cCase = installCase(M.StrCase(c, SOME caseid, restrictRel(rel, dom')))
                     in
                      	flattenCase'(cCase, installCase, true)
                     end
                  | M.SymMod _ => ()
     in
        M.sgnApp(dom, flatten1)
