...
sig
  ...
  *)
  ...
  (* signatures
     Sign m: a named signature
     SignUnion(s,t): a union of signatures, associative, commutative
     SignAlong(m,mor): pushout of m along mor, pushouts compose vertically and horizontally
  *)
  ...
  *)
  ...
  (* symbol instantiations within a view or structure
     ...
     * InclInst(c, O, R, mor) instantiates the direct inclusion into S from R) with the morphism mor from R to T
  ...
  *)
  ...
  datatype SigIncl
     = SigIncl of IDs.mid     (* included signature *)
                * bool        (* true iff the include defines the meta-theory *)     
                * OpenDec     (* opening and renaming of constants in the included signature *)
                * bool        (* true iff the include is generated by flattening or materialization *)     
  ...
  *)
  datatype ModDec
     = SigDec of
         URI.uri                       (* namespace *)
       * string list                   (* name *)
       * Sign option                   (* if generated by elaboration, the origin *)                                
     | ViewDec of
         URI.uri                       (* namespace *)
       * string list                   (* name *)
       * IDs.mid                       (* domain *)
       * IDs.mid                       (* codomain *)
       * Sign option                   (* unelaborated codomain *)
       * bool                          (* implicit *)
     | RelDec of
         URI.uri                       (* namespace *)
       * string list                   (* name *)
  ...
  val modDecOrg  : ModDec -> Sign option
  ...
  (* sigRelType describes signatures visible to a signature m: 
     ...
     c) every signature d included via include declaration with cid: (d, Included (cid, mt))
        special cases: mt = SOME Meta: meta-theory, mt = SOME MetaIncluded: included into meta-theory
     d) included signatures include transitive flattening
     ...
     The order in the list is: Self, Ancestor _ (innermost to outermost), AncIncluded (any order), Included _ (dependency order)
  ...
  (* index for efficiency: application of a structure to a constant/structure/inclusion
     in addition to efficiency, this is used for sharing: structMapLookup(s,i) = s' if s maps i to s' and s.i  does not exist  *)
  val structMapLookup : IDs.cid * IDs.cid -> IDs.cid option
  (* inserts an entry into the lookup table *)
  val structMapAdd : (IDs.cid * IDs.cid) * IDs.cid -> unit
