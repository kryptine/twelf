...
%implementation. 
%(including complex algorithms such as higher-order
%pattern unification). 
%In such cases, we have an option of regenerating
...
% of the term in the index, and show correctness. 
...
%We see that the three given clauses share a lot of structure. 
% For
...
% with $\eqilLF$ and $\eqirLF$. 
%To insert these three clauses into a
...
% left-most branch, we obtain the clause head $\eqalLF$.  b
...
%It is useful to pre-process the
...
%approach to caching.
%The idea can be briefly summarized
%as follows: when generating compressed witnesses for a goal $G$ from
%explicit proof terms, we store intermediate goals together with their
%solutions in a table, and re-use the result later on. For checking
%proof witnesses, we consult the oracle to see whether the tabled
%answer is to be used. Further, intermediate goals are stored similarly
...
allow for caching.
%by defining an auxiliary procedure {\sf
...
\end{small}
\end{center}
%\caption{\label{tab:reftimes} Refinement Type System : Proof Compression Times with Caching}
%\end{table*}
...
%
Finally, % when comparing proof size to the size of the compact
% witness , 
we notice that the compact oracle is about 1\% of the size of the
proof term.% (see Table~\ref{tab:reftimes}). 
...
terms.
%, as we show in Table~\ref{tab:fpcctimes}.
...
%We use linear higher-order patterns and higher-order substitution
%tree indexing to remove the non-determinism based on Huet's unification
%algorithm. 

%Moreover, our implementation scales to dependent
...
%Their checker follows the path explored by
%Necula and Rahul and ignores higher-order terms. 
