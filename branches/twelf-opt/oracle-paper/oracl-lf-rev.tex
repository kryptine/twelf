...
Harper {\sl et al}~\cite{Harper93jacm}. For more information on how to
...
are given goal $G$ in a program context $\Gamma$, together with a
proof witness $W$. The procedure is the dual of the compression case, and we show the important \mbox{{\bf Select}} case.
...
cached, and for those we will factor out common sub-proofs. This uses
ideas and also the infrastructure from tabled logic
programming~\cite{Pientka03phd,Pientka:ICLP02}. We will modify the
...
the oracle is in terms of the unary encoding scheme described earlier. The 
predicate selected corresponds to saying that a formula is on the right
of the sequent.
...
speedup between 3 and 6 times. This figure is achieved if we are caching
subgoals to get maximum compressions. As we see later, even more time gains
...
fairly expensive operation, in terms of both store and lookup time, as
well as the extra memory required to maintain the table. In
Table~\ref{tab:refcache} we investigate this tradeoff. We find that
using caching results in a speed penalty of between 3 and 16
