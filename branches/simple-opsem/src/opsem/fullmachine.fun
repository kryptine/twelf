...
struct
  ...
  *)
  fun gSolve ((C.Atom(p), s), dp, sc) = matchAtom ((p,s), dp, sc)
  ...
  *)
  and rSolve (ps', (C.Eq(Q), s), F.DProg (G, dPool), sc) =
    ...
    | rSolve (ps', (C.Exists(I.Dec(_,A), r), s), dp as F.DProg (G, dPool), sc) =
      let
	val X = I.newEVar (G, I.EClo (A,s))
      in
	rSolve (ps', (r, I.Dot(I.Exp(X), s)), dp,
  ...
  *)
  and matchAtom (ps' as (I.Root(I.Const(a),S),s), dp as F.DProg (G,dPool), sc) =
      let
        (* matchSig [c1,...,cn] = ()
    ...
    fun solve (q, sc) = qSolve ((q, I.id), F.DProg(I.Null, I.Null), sc)
